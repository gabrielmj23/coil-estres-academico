...
export const initPreguntasGoldberg = async () => {
  ...
  const preguntasExistentes = await db
    .select()
    .from(preguntas)
    .where(eq(preguntas.idCuestionario, 2));
  ...
  try {
    ...
    if (!goldberg.length || !ansDep.length || !disSocial.length) {
      console.error(
        "Faltan datos iniciales (Goldberg, Ansiedad/Depresión o Disfunción Social)."
      );
      return;
    }
    ...
    const preguntasGoldberg = [
      {
        ...
        posicion: 1,
        idIndicador: idDisfuncionSocial,
      },
      {
        contenido: "¿Sus preocupaciones le han hecho perder mucho sueño?",
        posicion: 2,
        idIndicador: idAnsiedadDepresion,
      },
      {
    ...
    console.error(
      "Error al inicializar preguntas para el cuestionario Goldberg:",
      error
    );
  }
};
...
export const initPreguntasSISCO = async () => {
  ...
  const preguntasExistentes = await db
    .select()
    .from(preguntas)
    .where(eq(preguntas.idCuestionario, 1));
